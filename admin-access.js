...
export async function requireAdmin(){return new Promise((resolve,reject)=>{const stop=onAuthStateChanged(auth,async user=>{stop();if(!user)return reject(new Error('not-authenticated'));try{const page=location.pathname.split('/').pop();let role='student';if((user.email||'').toLowerCase()===OWNER){role='owner'}else{const snap=await getDoc(doc(db,'users',user.uid));role=snap.data()?.role||'student'}const allowed=pageAccess[page]||['owner','admin'];if(!allowed.includes(role))return reject(new Error('not-authorized'));resolve({user,role})}catch(error){reject(error)}})})}
